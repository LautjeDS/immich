...
data class QueryResult(val assets: List<ImmichAsset>, val nextPageToken: String?)

object ImmichCloudRepository {
  private lateinit var appContext: Context
  private lateinit var syncPrefs: SharedPreferences
  private lateinit var serverUrlsJson: String
  ...
  fun initialize(context: Context) {
    ...
    val mainPrefs = appContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    serverUrlsJson = mainPrefs.getString(PREFS_SERVER_URLS, null) ?: "[]"
  }
  ...
  private fun getServerUrl(): String? {
    try {
      return Json.decodeFromString<List<String>>(serverUrlsJson)[0]
    } catch (e: Exception) {
      Log.e(TAG, "Failed to load server URLS from \"$serverUrlsJson\"", e)
      return null
    }
  }
  ...
  private fun getDatabaseFile(): File? {
    // drift_flutter stores the database in the app_flutter directory with .sqlite extension
  ...
  private fun refreshSyncGeneration() {
    try {
      ...
      if (count > 0) {
        ...
        syncPrefs.edit { putLong("sync_generation", syncGeneration) }
  ...
  private fun countAssetsFromDb(): Long {
    ...
    return try {
      ...
      cursor.use { if (it.moveToFirst()) it.getLong(0) else 0 }
    } catch (e: Exception) {
  ...
  fun getAccountName(): String {
    ...
    // Try reading from the local DB first
    val dbName = getAccountNameFromDb()
    if (dbName != null) {
      syncPrefs.edit { putString("account_name", dbName) }
      return dbName
    }
    ...
    return try {
      ...
      if (response.isSuccessful) {
        ...
        syncPrefs.edit { putString("account_name", accountName) }
        accountName
      } else {
        getServerUrl() ?: "Immich"
      }
  ...
  private fun getAccountNameFromDb(): String? {
    ...
    return try {
      // auth_user_entity has name and email for the logged-in user
      val cursor = db.rawQuery("SELECT name, email FROM auth_user_entity LIMIT 1", null)
      cursor.use {
        if (it.moveToFirst()) {
  ...
  fun updateMediaCollectionId(newId: String) {
    syncPrefs.edit { putString("media_collection_id", newId) }
  }
  ...
  fun queryAlbumAssets(
    ...
    LEFT JOIN remote_exif_entity e ON e.asset_id = r.id
    WHERE raa.album_id = ? AND r.deleted_at IS NULL
    ORDER BY COALESCE(r.local_date_time, r.created_at) DESC
    LIMIT ? OFFSET ?
   """,
          arrayOf(albumId, pageSize.toString(), offset.toString())
        )
  ...
  private fun assetFromCursor(c: android.database.Cursor): ImmichAsset {
    ...
    // 1=image, 2=video per AssetType enum
    val typeInt = c.getInt(1)
    // Drift stores as ISO-8601 text (storeDateTimeAsText: true)
    val createdAtStr = c.getString(2)
    val width = if (c.isNull(3)) 0 else c.getInt(3)
    val height = if (c.isNull(4)) 0 else c.getInt(4)
    val durationMs = if (c.isNull(5)) 0L else c.getLong(5)
    ...
    val dateTakenMillis =
      try {
        ...
        try {
          // Drift may store without trailing Z
  ...
  fun getAssetById(assetId: String): ImmichAsset? {
   ...
   """,
      ...
      cursor.use { if (it.moveToFirst()) assetFromCursor(it) else null }
    } catch (e: Exception) {
  ...
  fun queryDeletedAssets(syncGeneration: Long): List<String> {
    ...
    return try {
      ...
      Log.d(
        TAG,
        "queryDeletedAssets: previous=${previousIds.size}, current=${currentIds.size}, deleted=${deleted.size}"
      )
      deleted.toList()
    } catch (e: Exception) {
  ...
  fun detectAndApplyChanges() {
    ...
    try {
      ...
      if (previousIds.isNotEmpty() && currentIds != previousIds) {
        ...
        Log.d(
          TAG,
          "detectAndApplyChanges: asset set changed (prev=${previousIds.size}, curr=${currentIds.size}), bumped syncGen to $syncGeneration"
        )
      }
  ...
  fun queryAlbums(): List<ImmichAlbum> {
    ...
    return try {
      val cursor =
        db.rawQuery(
          """
    SELECT a.id, a.name, a.thumbnail_asset_id, a.updated_at,
       COUNT(raa.asset_id) AS asset_count
    FROM remote_album_entity a
    LEFT JOIN remote_album_asset_entity raa ON raa.album_id = a.id
    GROUP BY a.id
   ...
   """,
      ...
      cursor.use {
        while (it.moveToNext()) {
          val updatedAtStr = it.getString(3)
          val dateTakenMillis =
            try {
              java.time.Instant.parse(updatedAtStr).toEpochMilli()
            } catch (_: Exception) {
              try {
                java.time.LocalDateTime.parse(updatedAtStr)
                  .atZone(java.time.ZoneOffset.UTC)
                  .toInstant()
                  .toEpochMilli()
              } catch (_: Exception) {
          ...
          albums.add(
            ImmichAlbum(
              ...
              coverAssetId = if (it.isNull(2)) null else it.getString(2),
              dateTakenMillis = dateTakenMillis,
              mediaCount = it.getInt(4)
            )
          )
        }
      }
      Log.d(TAG, "queryAlbums: returning ${albums.size} albums from DB")
      albums
    } catch (e: Exception) {
  ...
  private fun downloadToTempFile(request: Request, prefix: String): ParcelFileDescriptor? {
    return try {
      ...
      val response = getClient().newCall(request).execute()
      if (!response.isSuccessful) {
        Log.e(TAG, "$id: Download to temp failed: ${response.code}")
        response.close()
        return null
      }
